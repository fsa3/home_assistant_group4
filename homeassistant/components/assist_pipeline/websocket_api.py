...
def websocket_list_languages(
    ...
    # Helper function to process language tags and update pipeline_languages
    def process_language_tags(
        language_tags: set[str] | None, current_languages: set[str] | None
    ) -> set[str] | None:
        ...
        if current_languages is not None:
            return language_util.intersect(current_languages, languages)
        return languages

    # Process conversation, STT, and TTS language tags
    ...
    if conv_language_tags and conv_language_tags != MATCH_ALL:
        pipeline_languages = process_language_tags(
            conv_language_tags, pipeline_languages
        )

    stt_language_tags = stt.async_get_speech_to_text_languages(hass)
    pipeline_languages = process_language_tags(stt_language_tags, pipeline_languages)

    tts_language_tags = tts.async_get_text_to_speech_languages(hass)
    pipeline_languages = process_language_tags(tts_language_tags, pipeline_languages)
