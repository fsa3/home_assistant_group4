...
class AuthStore:
    ...
    async def async_load(self) -> None:  # noqa: C901
        ...
        self._load_credentials(data, users, credentials)
    ...
    def _load_refresh_tokens(
        ...
        """Load refresh tokens from stored data."""
        for rt_dict in data["refresh_tokens"]:
            token = self._process_refresh_token(rt_dict, users, credentials)
            if token is not None:
                users[rt_dict["user_id"]].refresh_tokens[token.id] = token

    def _process_refresh_token(
        self,
        rt_dict: dict[str, Any],
        users: dict[str, models.User],
        credentials: dict[str, models.Credentials],
    ) -> models.RefreshToken | None:
        """Process an individual refresh token."""
